...
const PricingAndHowItWorks = () => {
  ...
  // Plans array is kept as is (though your code uses transparentData for rendering)
  const plans = [
    {
      name: "Starter",
      price: "$29",
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            {transparentData?.slice(0, 3).map((ele, index) => (
              <motion.div 
                key={ele.id} 
                className="z-10 py-2" // Reduced py here to fit animation better
                variants={cardVariants}
              >
                {/* 🔥 3D Card Effect on Hover */}
                <motion.div 
                  className="bg-white rounded-xl border border-gray-200 w-full mx-auto shadow-md h-full flex flex-col"
                  whileHover={{ 
                    y: -15, // Lift effect
                    scale: 1.02, // Slight zoom
                    boxShadow: "0px 25px 50px -12px rgba(48, 92, 222, 0.25)" // Premium blue shadow
                  }}
                ...
                >
                  ...
                  <div className="p-4 mt-auto">
                    <motion.button
                      className="w-full py-3 rounded-lg font-medium transition cursor-pointer bg-[#305CDE] text-white hover:bg-blue-700"
                      onClick={() => handleUrl()}
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="mt-12 cursor-pointer bg-[#305CDE] text-white px-8 py-3 rounded-lg font-medium hover:bg-blue-700 transition shadow-lg hover:shadow-blue-500/30"
          >
            View All plans →
          </motion.button>
