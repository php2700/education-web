...
const SBACPrep = () => {
  ...
  return (
    <div className="w-full bg-white text-gray-800" id='sbac'>
      ...
      {/* ASSESSMENT DETAILS */}
      {/* Render only if content exists */}
      {(safeData.assessmentHeading || (safeData.assessmentPoints && safeData.assessmentPoints.length > 0)) && (
        <section className="bg-gray-100 py-16 px-4">
            <div className="max-w-7xl mx-auto">
            ...
            {safeData.assessmentPoints && safeData.assessmentPoints.length > 0 && (
                <ul className="list-disc pl-5 space-y-3 text-lg">
                    {safeData.assessmentPoints.map((point, index) => (
      ...
      {/* ACCESSIBILITY */}
      {/* Render only if content exists */}
      {(safeData.accessHeading || (safeData.accessPoints && safeData.accessPoints.length > 0)) && (
        <section className="py-16 px-4">
            <div className="max-w-7xl mx-auto">
            {safeData.accessHeading && (
                <h2 className="text-3xl font-bold mb-6">
                    {safeData.accessHeading}
                </h2>
            )}

            {safeData.accessPoints && safeData.accessPoints.length > 0 && (
                <ul className="list-disc pl-5 space-y-3 text-lg">
                    {safeData.accessPoints.map((point, index) => (
                        point ? <li key={index}>{point}</li> : null
                    ))}
                </ul>
            )}
