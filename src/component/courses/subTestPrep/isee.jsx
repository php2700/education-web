...
//         <h1 className="text-4xl font-bold text-center text-blue-900 mb-12">
//            {/* Admin Key: heroTitle */}
//            {safeData.heroTitle || "ISEE Test ghh Prep"}
//         </h1>
    ...
    <div className="bg-[#F0F8FF] py-14 px-4" id='isee'>
      <div className="max-w-7xl mx-auto">
        ...
        {(safeData.structureHeading || safeData.structureLevelIntro || safeData.structureSectionIntro) && (
            ...
            {safeData.structureLevels && safeData.structureLevels.length > 0 && (
                <ul className="list-disc ml-6 text-gray-700 space-y-1 mb-6">
                    {safeData.structureLevels.map((pt, i) => (
                        pt ? <li key={i}>{pt}</li> : null
                    ))}
                </ul>
            )}
            ...
            {safeData.structureSections && safeData.structureSections.length > 0 && (
                <ul className="list-disc ml-6 text-gray-700 space-y-1">
                    {safeData.structureSections.map((pt, i) => (
                        pt ? <li key={i}>{pt}</li> : null
                    ))}
                </ul>
            )}
        ...
        {/* ===== 5. SECTION DETAILS (Measures) ===== */}
        {(safeData.measureHeading || (safeData.measurePoints && safeData.measurePoints.length > 0)) && (
            <div className="bg-white p-8 md:p-10 rounded-2xl shadow-md mb-12">
