...
export const Blog = () => {
  ...
  return (
    <div
      className="bg-[#F0F8FF] py-16 px-6 sm:px-10 lg:px-12 relative overflow-hidden min-h-screen"
      style={{
        backgroundImage: `url(${backgroundImage})`,
        backgroundSize: "contain",
        backgroundPosition: "center",
    ...
    >
      ...
      {/* Logic: Loader ke baad hi content dikhega aur animate hoga */}
      {!loading && blogData.length > 0 ? (
        <motion.div 
          key={blogData.length} // Key lagane se data aate hi component refresh hokar animate karega
          className="container mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"
          variants={containerVariants}
          initial="hidden"
          animate="visible" // "whileInView" ki jagah "animate" use kiya taaki turant chale
          viewport={{ once: true, amount: 0.1 }}
        >
          {blogData.map((blog) => (
            <motion.div
        ...
        // Empty state ya loader ke liye space
        <div className="h-64 flex items-center justify-center">
          {loading && <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-blue-600"></div>}
        </div>
      )}
    </div>
  );
